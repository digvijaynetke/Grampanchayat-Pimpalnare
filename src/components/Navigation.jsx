...
const Navigation = ({ onOpenComplaint }) => {
  ...
  <div className="text-2xl md:text-3xl">ग्रामपंचायत पिंपळणारे</div>
            ...
            <ul className="flex items-center gap-5">
              {menuItems.map((item, index) => (
                <li key={index} className="relative group">
                  <a
                    href={item.link || '#'}
                    className="text-gray-700 hover:text-teal-600 transition py-2 block text-lg md:text-xl font-medium"
                  >
                    {item.label}
                  </a>
                  {item.submenu && (
